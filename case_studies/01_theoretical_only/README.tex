...
\title{Case Study 01 — Theoretical Only}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Canonical TD-DFT-only workflow}
\author{}
\date{}
...
\hypertarget{case-study-01--theoretical-only}{
\section{Case Study 01 --- Theoretical
Only}\label{case-study-01--theoretical-only}}

Run \texttt{overlap-calculator} on Gaussian TD-DFT outputs only, with no
measured spectra in the manifest. This is the canonical theoretical
workflow and the place to start if this is your first case study --- the
prerequisites section here is referenced from every other case.
...
\hypertarget{11-install-the-package-pick-one}{
\subsubsection{1.1 Install the package (pick
one)}\label{11-install-the-package-pick-one}}

\textbf{Conda (recommended).} From the repository root, run each command
in a terminal:
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{conda}\NormalTok{ activate overlap{-}calculator}
\end{Highlighting}
\end{Shaded}
...
\textbf{Or, plain venv.} From the repository root:
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{.}\FunctionTok{venv}\NormalTok{\textbackslash{}Scripts\textbackslash{}Activate}\OperatorTok{.}\FunctionTok{ps1}
\end{Highlighting}
\end{Shaded}
...
Both paths put the \texttt{overlap-calculator} command on your
\texttt{PATH}. Both the CLI and the API run from the same Python
environment.

\hypertarget{12-install-the-helper-script-dependency}{
\subsubsection{1.2 Install the helper script
dependency}\label{12-install-the-helper-script-dependency}}

The Python helper
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/01_theoretical_only/submit.py}{\texttt{submit.py}}
talks to the HTTP API, which needs the \texttt{requests} package:
...
\hypertarget{13-start-the-http-api}{
\subsubsection{1.3 Start the HTTP API}\label{13-start-the-http-api}}

In a \textbf{separate} terminal at the repository root, start the API:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ overlap\_calculator.api.app}
\end{Highlighting}
\end{Shaded}

The API now listens on \texttt{http://localhost:8000}. Leave that
terminal running for the rest of the case study.
...
That publishes the API on host port \texttt{8080}. Tell
\texttt{submit.py} to target the Docker port:

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{export} \VariableTok{OVERLAP\_API\_URL=}\NormalTok{http://localhost:8080/analyze}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{$env}\OperatorTok{:}\NormalTok{OVERLAP\_API\_URL }\OperatorTok{=} \StringTok{"http://localhost:8080/analyze"}
\end{Highlighting}
\end{Shaded}

\hypertarget{14-verify-the-api-is-reachable}{
\subsubsection{1.4 Verify the API is
reachable}\label{14-verify-the-api-is-reachable}}

In any terminal:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curl}\NormalTok{ http://localhost:8000/health}
\end{Highlighting}
\end{Shaded}
...
If you started the API via \texttt{docker\ compose}, swap port
\texttt{8000} for \texttt{8080} in the \texttt{curl} URL.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/01\_theoretical\_only/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/01\_theoretical\_only/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run, \texttt{case\_studies/01\_theoretical\_only/output/cli/}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/01\_theoretical\_only/submit.py}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/02_experimental_only/README.md}{Case
  Study 02 --- Experimental Only}
\item
...
\item
  \href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/06_plot_dpi/README.md}{Case
  Study 06 --- Plot DPI}
\item
