"""Theoretical-only case study: drive both the CLI and the HTTP API.

Runs ``overlap-calculator analyze`` against the local manifest, then
submits the same TD-DFT files to the running HTTP API at ``API_URL``.
