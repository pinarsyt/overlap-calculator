"""Experimental-only case study: drive both the CLI and the HTTP API.

Runs ``overlap-calculator analyze`` on the bundled PhotochemCAD-style
Excel workbook (8 numeric absorbance series), then submits the same
file to the running HTTP API at ``API_URL``.
