...
Build one manifest that contains both Gaussian TD-DFT outputs and experimental Excel spectra, so computed and measured absorption results land in the same `results.csv` under the same broadening, wavelength grid, and Beer–Lambert reference. Each row is tagged with `source_type ∈ {"theoretical", "experimental"}`.

**Citation.** The bundled experimental data is derived from M. Taniguchi & J. S. Lindsey, *Photochem. Photobiol.* **94** (2018) 290–327. See the project [README](../../README.md#acknowledgement-of-third-party-data) for the full citation.
...
The case-study `input.json` lists 2 TD-DFT outputs and 2 absorbance series from 1 Excel workbook. The manifest uses
...
```bash
overlap-calculator analyze --input case_studies/03_mixed_theory_experiment/input.json --out case_studies/03_mixed_theory_experiment/output/cli --default-light-sources AM15G,LEDB4 --plot-dpi 400
```

After the run, `case_studies/03_mixed_theory_experiment/output/cli/` contains a
...
```bash
curl -X POST http://localhost:8000/analyze -F "files=@input/files/slurm-5473089.out" -F "files=@input/files/slurm-5473178.out" -F "files=@input/files/organic_uvvis_photochemcad_dataset.xlsx" -F "default_light_sources=AM15G,LEDB4" -F "plot_dpi=400" --output case_studies/03_mixed_theory_experiment/output/api/analysis_outputs.zip
```
...
```bash
python case_studies/03_mixed_theory_experiment/submit.py
```
...
```text
case_studies/03_mixed_theory_experiment/output/
+-- cli/
|   +-- tables/results.{csv,json,xlsx}        (4 samples (2 theoretical + 2 experimental) × 2 light sources = 8 rows)
