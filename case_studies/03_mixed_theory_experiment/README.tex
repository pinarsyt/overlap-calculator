...
\title{Case Study 03 — Mixed Theory + Experiment}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{TD-DFT and measured spectra in one manifest}
\author{}
\date{}
...
\hypertarget{case-study-03--mixed-theory--experiment}{
\section{Case Study 03 --- Mixed Theory +
Experiment}\label{case-study-03--mixed-theory--experiment}}

Build one manifest that contains both Gaussian TD-DFT outputs and
experimental Excel spectra, so computed and measured absorption results
land in the same \texttt{results.csv} under the same broadening,
wavelength grid, and Beer--Lambert reference. Each row is tagged with
\texttt{source\_type} \(\in \{\)\texttt{"theoretical"}, \texttt{"experimental"}\(\}\).

\textbf{Citation.} The bundled experimental data is derived from M.
Taniguchi \& J. S. Lindsey, \emph{Photochem. Photobiol.} \textbf{94}
...
The case-study \texttt{input.json} lists 2 TD-DFT outputs and 2
absorbance series from 1 Excel workbook. The manifest uses relative
paths that point at the bundled data under
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/03\_mixed\_theory\_experiment/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/03\_mixed\_theory\_experiment/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run,
\texttt{case\_studies/03\_mixed\_theory\_experiment/output/cli/}
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/03_mixed_theory_experiment/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/03\_mixed\_theory\_experiment/submit.py}
\end{Highlighting}
\end{Shaded}
...
+-- cli/
|   +-- tables/results.{csv,json,xlsx}        (4 samples (2 theoretical + 2 experimental) x 2 light sources = 8 rows)
