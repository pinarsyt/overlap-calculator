"""Mixed theory + experiment case study: drive both CLI and HTTP API.

Runs ``overlap-calculator analyze`` on the 13-dye TD-DFT library plus
the 8 numeric absorbance series from the bundled PhotochemCAD workbook
in a single manifest, then submits the same files to the running HTTP
API at ``API_URL``.
