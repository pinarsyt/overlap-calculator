...
\title{Case Study 04 — Custom Light Source}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{User-supplied light-source CSV}
\author{}
\date{}
...
\hypertarget{case-study-04--custom-light-source}{
\section{Case Study 04 --- Custom Light
Source}\label{case-study-04--custom-light-source}}

Add a \textbf{user-supplied} reference light source on top of one
bundled built-in (\texttt{AM15G}), so you can directly compare how the
same dye absorbs under both. The user-supplied source here is the
bundled CIE D65 daylight illuminant, which is \emph{not} in the default
light-source list.
...
\tightlist
\item
  CLI: \texttt{-\/-light-source-file\ PATH} (repeatable for multiple
  files)
\item
  API: a \texttt{light\_source\_files} multipart upload (also
  repeatable)
\end{itemize}

A custom light-source CSV must contain exactly two columns:
\texttt{wavelength\_nm} and \texttt{intensity} (with that header). The
filename stem becomes the \texttt{light\_source\_name} in the result
table.
...
Light sources used: \textbf{AM1.5G + custom CIE D65}.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/04\_custom\_light\_source/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/04\_custom\_light\_source/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G }\AttributeTok{{-}{-}light{-}source{-}file}\NormalTok{ src/overlap\_calculator/data/CIE\_std\_illum\_D65.csv }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run,
\texttt{case\_studies/04\_custom\_light\_source/output/cli/} contains a
\texttt{tables/} directory (and a \texttt{plots/} directory unless plots
were disabled). The \texttt{-\/-default-light-sources} flag pins the
bundled sources to AM1.5G + custom CIE D65; remove it to compare against
the full default set (\texttt{AM15G,LEDB4,LEDB2,LEDB3,CIEFL10}).
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curl} \AttributeTok{{-}X}\NormalTok{ POST http://localhost:8000/analyze }\AttributeTok{{-}F} \StringTok{"files=@input/files/slurm{-}5473089.out"} \AttributeTok{{-}F} \StringTok{"files=@input/files/slurm{-}5473178.out"} \AttributeTok{{-}F} \StringTok{"light\_source\_files=@src/overlap\_calculator/data/CIE\_std\_illum\_D65.csv"} \AttributeTok{{-}F} \StringTok{"default\_light\_sources=AM15G"} \AttributeTok{{-}F} \StringTok{"plot\_dpi=400"} \AttributeTok{{-}{-}output}\NormalTok{ case\_studies/04\_custom\_light\_source/output/api/analysis\_outputs.zip}
\end{Highlighting}
\end{Shaded}
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/04_custom_light_source/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/04\_custom\_light\_source/submit.py}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/05_default_light_source_selection/README.md}{Case
  Study 05 --- Default Light Source Selection}
\item
