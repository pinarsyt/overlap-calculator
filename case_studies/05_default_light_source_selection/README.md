# Case Study 05 — Default Light Source Selection

Pick a **subset** of the bundled built-in light sources rather than running against the full default set. Here we run against `LEDB1` and `LEDB4` — the former is *not* in the default set, so it is never analysed unless you opt into it explicitly.

The user-selectable knob is:

- CLI: `--default-light-sources NAME[,NAME...]`
- API: form field `default_light_sources=NAME[,NAME...]`

Allowed names: `AM15G`, `LEDB1`, `LEDB2`, `LEDB3`, `LEDB4`, `CIEFL10`.
...
Light sources used: **LED-B1 + LED-B4**.
...
```bash
overlap-calculator analyze --input case_studies/05_default_light_source_selection/input.json --out case_studies/05_default_light_source_selection/output/cli --default-light-sources LEDB1,LEDB4 --plot-dpi 400
```

After the run, `case_studies/05_default_light_source_selection/output/cli/` contains a
`tables/` directory (and a `plots/` directory unless plots were
disabled). The `--default-light-sources` flag pins the bundled sources
to LED-B1 + LED-B4; remove it to compare against the full default set
...
```bash
curl -X POST http://localhost:8000/analyze -F "files=@input/files/slurm-5473089.out" -F "files=@input/files/slurm-5473178.out" -F "default_light_sources=LEDB1,LEDB4" -F "plot_dpi=400" --output case_studies/05_default_light_source_selection/output/api/analysis_outputs.zip
```
...
```bash
python case_studies/05_default_light_source_selection/submit.py
```
...
- [Case Study 04 — Custom Light Source](../04_custom_light_source/README.md)
