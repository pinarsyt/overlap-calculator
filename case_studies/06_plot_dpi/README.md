...
Raise the TIFF plot resolution from the default `400` to `600` for **publication-resolution** exports. The result tables are unchanged; only the rendered plots get larger and sharper, at the cost of larger output files and longer plot time.

The user-selectable knob is:

- CLI: `--plot-dpi N` (minimum `72`)
- API: form field `plot_dpi=N`

The default is `400`. `600` is the recommended publication preset. Raising `--plot-dpi` does not change any numerical descriptor; it only changes the rasterisation of every emitted TIFF.
...
```bash
overlap-calculator analyze --input case_studies/06_plot_dpi/input.json --out case_studies/06_plot_dpi/output/cli --default-light-sources AM15G,LEDB4 --plot-dpi 600
```

After the run, `case_studies/06_plot_dpi/output/cli/` contains a
...
```bash
curl -X POST http://localhost:8000/analyze -F "files=@input/files/slurm-5473089.out" -F "files=@input/files/slurm-5473178.out" -F "default_light_sources=AM15G,LEDB4" -F "plot_dpi=600" --output case_studies/06_plot_dpi/output/api/analysis_outputs.zip
```
...
```bash
python case_studies/06_plot_dpi/submit.py
```
...
```text
case_studies/06_plot_dpi/output/
+-- cli/
|   +-- tables/results.{csv,json,xlsx}        (2 samples × 2 light sources = 4 rows (TIFF at 600 dpi))
...
|   +-- plots/                                                      (also plots/ranking/by_light_source/<metric>/ and plots/ranking/by_sample/<metric>/, all at 600 dpi)
+-- api/
...
- [Case Study 07 — Table Only (No Plots)](../07_table_only/README.md)
