...
\makeatother
\subtitle{Publication-resolution TIFF exports}
\author{}
\date{}
...
\hypertarget{case-study-06--plot-dpi}{
\section{Case Study 06 --- Plot DPI}\label{case-study-06--plot-dpi}}

Raise the TIFF plot resolution from the default \texttt{400} to
\texttt{600} for \textbf{publication-resolution} exports. The result
tables are unchanged; only the rendered plots get larger and sharper, at
the cost of larger output files and longer plot time.
...
\tightlist
\item
  CLI: \texttt{-\/-plot-dpi\ N} (minimum \texttt{72})
\item
  API: form field \texttt{plot\_dpi=N}
\end{itemize}

The default is \texttt{400}. \texttt{600} is the recommended publication
preset. Raising \texttt{-\/-plot-dpi} does not change any numerical
descriptor; it only changes the rasterisation of every emitted TIFF.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/06\_plot\_dpi/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/06\_plot\_dpi/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 600}
\end{Highlighting}
\end{Shaded}

After the run, \texttt{case\_studies/06\_plot\_dpi/output/cli/} contains
a \texttt{tables/} directory (and a \texttt{plots/} directory unless
plots were disabled). The \texttt{-\/-default-light-sources} flag pins
the bundled sources to AM1.5G + LED-B4; remove it to compare against the
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/06_plot_dpi/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/06\_plot\_dpi/submit.py}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/07_table_only/README.md}{Case
  Study 07 --- Table Only (No Plots)}
\item
