...
\title{Case Study 07 — Table Only (No Plots)}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Fast tabular screening without plots}
\author{}
\date{}
...
\hypertarget{case-study-07--table-only-no-plots}{
\section{Case Study 07 --- Table Only (No
Plots)}\label{case-study-07--table-only-no-plots}}

Disable plot generation entirely. Only the result tables are written.
Use it for \textbf{fast tabular screening} when you only need numerical
descriptors and not the rendered TIFF figures.
...
\tightlist
\item
  CLI: \texttt{-\/-no-plot-outputs} (the default is
  \texttt{-\/-plot-outputs})
\item
  API: form field \texttt{plot\_outputs=false}
\end{itemize}

Plot rendering dominates wall-clock time. With plots disabled, the same
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/07\_table\_only/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/07\_table\_only/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}no{-}plot{-}outputs}
\end{Highlighting}
\end{Shaded}

After the run, \texttt{case\_studies/07\_table\_only/output/cli/}
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/07_table_only/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/07\_table\_only/submit.py}
\end{Highlighting}
\end{Shaded}
