"""Table-only case study: drive both the CLI and the HTTP API.

Demonstrates skipping plot generation for fast tabular screening, via
``--no-plot-outputs`` on the CLI and ``plot_outputs=false`` on the API.

Outputs (single tree per case study):

- ``output/cli/``                       CLI tables (no ``plots/``)
- ``output/api/``                       API tables extracted from the ZIP
