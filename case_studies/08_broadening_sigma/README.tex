...
\title{Case Study 08 — Broadening Sigma}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Line-shape width sensitivity}
\author{}
\date{}
...
\hypertarget{case-study-08--broadening-sigma}{
\section{Case Study 08 --- Broadening
Sigma}\label{case-study-08--broadening-sigma}}

Sharpen the spectral broadening from the default \(\sigma = 0.30\,\text{eV}\)
down to \(\sigma = 0.20\,\text{eV}\). Sharper broadening produces taller,
narrower peaks in the molar absorptivity \(\varepsilon(\lambda)\), which can shift
\texttt{lambda\_max\_nm} only slightly but change
\texttt{absorbed\_fraction} markedly when the peak sits near the edge of
a light source's emission band.
...
\tightlist
\item
  CLI: \texttt{-\/-sigma-ev\ FLOAT} (typical range 0.1--0.4 eV)
\item
  API: form field \texttt{sigma\_ev=FLOAT}
\end{itemize}

Both Gaussian and Lorentzian broadening use the same \(\sigma\); the only
difference is the line shape itself.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/08\_broadening\_sigma/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/08\_broadening\_sigma/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}sigma{-}ev}\NormalTok{ 0.20 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run, \texttt{case\_studies/08\_broadening\_sigma/output/cli/}
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/08_broadening_sigma/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/08\_broadening\_sigma/submit.py}
\end{Highlighting}
\end{Shaded}
...
+-- cli/
|   +-- tables/results.{csv,json,xlsx}        (2 samples x 2 light sources = 4 rows (sigma = 0.20 eV))
...
\tightlist
\item
  \href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/09_beer_lambert_tuning/README.md}{Case
  Study 09 --- Beer--Lambert Tuning}
\item
