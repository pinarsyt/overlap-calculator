...
\title{Case Study 09 — Beer-Lambert Tuning}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Reference concentration and path length}
\author{}
\date{}
...
\hypertarget{case-study-09--beerlambert-tuning}{
\section{Case Study 09 --- Beer--Lambert
Tuning}\label{case-study-09--beerlambert-tuning}}

Change the \textbf{reference concentration} and \textbf{path length}
used in the Beer--Lambert step. Both knobs enter the absorptance only
through their product, \(c\cdot L\). Here we raise the concentration
by a factor of 10 (\texttt{1e-5\ M\ →\ 1e-4\ M}) and shorten the path by
a factor of 10 (\texttt{1.0\ cm\ →\ 0.1\ cm}), so the product stays the
same.
...
\tightlist
\item
  CLI: \texttt{-\/-concentration-m\ FLOAT} (mol L\(^{-1}\)) and
  \texttt{-\/-path-cm\ FLOAT} (cm)
\item
  API: form fields \texttt{concentration\_m=FLOAT} and
  \texttt{path\_cm=FLOAT}
\end{itemize}

The defaults are \texttt{1e-5\ M} and \texttt{1.0\ cm}, giving a product
of \(1\times 10^{-5}\,\text{mol\,L}^{-1}\cdot\text{cm}\) that keeps
\texttt{absorptance\_max} below \textasciitilde0.9 for most of the
bundled dyes.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/09\_beer\_lambert\_tuning/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/09\_beer\_lambert\_tuning/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}concentration{-}m}\NormalTok{ 1e{-}4 }\AttributeTok{{-}{-}path{-}cm}\NormalTok{ 0.1 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run,
\texttt{case\_studies/09\_beer\_lambert\_tuning/output/cli/} contains a
...
bundled sources to AM1.5G + LED-B4; remove it to compare against the
full default set (\texttt{AM15G,LEDB4,LEDB2,LEDB3,CIEFL10}).
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/09_beer_lambert_tuning/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/09\_beer\_lambert\_tuning/submit.py}
\end{Highlighting}
\end{Shaded}
...
+-- cli/
|   +-- tables/results.{csv,json,xlsx}        (2 samples x 2 light sources = 4 rows (c = 1e-4 M, L = 0.1 cm))
...
\item
  \href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/10_wavelength_grid/README.md}{Case
  Study 10 --- Wavelength Grid}
\item
