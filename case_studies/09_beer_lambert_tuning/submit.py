"""Beer-Lambert tuning case study: drive both the CLI and the HTTP API.

Demonstrates raising the reference concentration (``1e-5`` -> ``1e-4`` M)
and shortening the path length (``1.0`` -> ``0.1`` cm) so that the
product ``c * L`` stays the same but the saturation regime is exposed.
Uses ``--concentration-m`` / ``--path-cm`` on the CLI and
``concentration_m`` / ``path_cm`` on the API.
