...
\title{Case Study 10 — Wavelength Grid}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Working grid bounds and density}
\author{}
\date{}
...
\hypertarget{case-study-10--wavelength-grid}{
\section{Case Study 10 --- Wavelength
Grid}\label{case-study-10--wavelength-grid}}

Widen the working wavelength grid from the default \texttt{200..800\ nm}
with \texttt{10000} points to \texttt{250..1100\ nm} with \texttt{20000}
points. Use a wider grid when your dyes absorb (or your light sources
emit) outside the default window --- for instance NIR dyes, or solar
spectra extending into the IR.
...
\tightlist
\item
  CLI: \texttt{-\/-wl-min\ FLOAT}, \texttt{-\/-wl-max\ FLOAT},
  \texttt{-\/-num-points\ INT}
\item
  API: form fields \texttt{wl\_min}, \texttt{wl\_max},
  \texttt{num\_points}
\end{itemize}

The grid is shared across all samples and all light sources in the same
run.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/10\_wavelength\_grid/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/10\_wavelength\_grid/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}wl{-}min}\NormalTok{ 250 }\AttributeTok{{-}{-}wl{-}max}\NormalTok{ 1100 }\AttributeTok{{-}{-}num{-}points}\NormalTok{ 20000 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run, \texttt{case\_studies/10\_wavelength\_grid/output/cli/}
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/10_wavelength_grid/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/10\_wavelength\_grid/submit.py}
\end{Highlighting}
\end{Shaded}
