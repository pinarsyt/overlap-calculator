...
Pick a **specific Excel sheet by name** when an input workbook has more than one sheet. The same logical input is expressed two slightly different ways depending on the interface.

The user-selectable knob is:

- CLI: each entry in [`input.json`](input.json) sets `sheet_name` explicitly. There is no separate flag.
- API: the `/analyze` endpoint accepts a `sheet_overrides` form field — a JSON object mapping each uploaded filename to the sheet name it should be parsed under.

**Citation.** The bundled workbook is derived from M. Taniguchi & J. S. Lindsey, *Photochem. Photobiol.* **94** (2018) 290–327. See the project [README](../../README.md#acknowledgement-of-third-party-data) for the full citation.
...
```bash
overlap-calculator analyze --input case_studies/11_sheet_overrides/input.json --out case_studies/11_sheet_overrides/output/cli --default-light-sources AM15G,LEDB4 --plot-dpi 400
```

After the run, `case_studies/11_sheet_overrides/output/cli/` contains a
...
```bash
curl -X POST http://localhost:8000/analyze -F "files=@input/files/organic_uvvis_photochemcad_dataset.xlsx" -F 'sheet_overrides={"organic_uvvis_photochemcad_dataset.xlsx":"program_input_absorbance"}' -F "default_light_sources=AM15G,LEDB4" -F "plot_dpi=400" --output case_studies/11_sheet_overrides/output/api/analysis_outputs.zip
```
...
```bash
python case_studies/11_sheet_overrides/submit.py
```
...
```text
case_studies/11_sheet_overrides/output/
+-- cli/
