...
\title{Case Study 11 — Sheet Overrides}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Excel sheet selection per upload}
\author{}
\date{}
...
\hypertarget{case-study-11--sheet-overrides}{
\section{Case Study 11 --- Sheet
Overrides}\label{case-study-11--sheet-overrides}}

Pick a \textbf{specific Excel sheet by name} when an input workbook has
more than one sheet. The same logical input is expressed two slightly
different ways depending on the interface.
...
\item
  CLI: each entry in
  \href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/11_sheet_overrides/input.json}{\texttt{input.json}}
  sets \texttt{sheet\_name} explicitly. There is no separate flag.
\item
  API: the \texttt{/analyze} endpoint accepts a
  \texttt{sheet\_overrides} form field --- a JSON object mapping each
  uploaded filename to the sheet name it should be parsed under.
\end{itemize}

\textbf{Citation.} The bundled workbook is derived from M. Taniguchi \&
J. S. Lindsey, \emph{Photochem. Photobiol.} \textbf{94} (2018) 290--327.
See the project
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/11\_sheet\_overrides/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/11\_sheet\_overrides/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run, \texttt{case\_studies/11\_sheet\_overrides/output/cli/}
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/11_sheet_overrides/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/11\_sheet\_overrides/submit.py}
\end{Highlighting}
\end{Shaded}
