...
\title{Case Study 12 --- Frequency-Resolved Prefactor}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Switching the oscillator-strength \(\to\) \(\varepsilon\) convention}
\author{}
\date{}
...
\hypertarget{case-study-12--frequency-resolved-prefactor}{
\section{Case Study 12 --- Frequency-Resolved
Prefactor}\label{case-study-12--frequency-resolved-prefactor}}

Switch the oscillator-strength \(\to\) \(\varepsilon\) convention from
the default \texttt{constant} mode to \texttt{frequency-resolved}. In
the frequency-resolved mode the wavenumber factor is kept inside the
integrand:
\[
\varepsilon(\nu) = \sum_i P f_i \frac{\nu}{\nu_i}\, g_i(\nu - \nu_i).
\]
...
\(O((\sigma/\nu_i)^2)\) --- below \(\sim\)2\,\% over 200--800\,nm and
growing into the NIR.

The user-selectable knob is:
\begin{itemize}
\tightlist
\item
  CLI: \texttt{-{}-prefactor-mode frequency-resolved}
\item
  API: form field \texttt{prefactor\_mode=frequency-resolved}
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/12\_frequency\_resolved\_prefactor/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/12\_frequency\_resolved\_prefactor/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}prefactor{-}mode}\NormalTok{ frequency-resolved }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run,
\texttt{case\_studies/12\_frequency\_resolved\_prefactor/output/cli/}
contains a \texttt{tables/} directory and a \texttt{plots/} directory.
The \texttt{results.*} tables carry a \texttt{prefactor\_mode} column
confirming the value used. To compare directly with the default, run the
same command without \texttt{-{}-prefactor-mode frequency-resolved} and
diff the two \texttt{descriptor\_summary.*} files.
...
\href{https://github.com/pinarsyt/overlap-calculator/blob/main/case_studies/12_frequency_resolved_prefactor/submit.py}{\texttt{submit.py}}
from the repository root:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/12\_frequency\_resolved\_prefactor/submit.py}
\end{Highlighting}
\end{Shaded}
...
Open \texttt{tables/results.csv} and inspect the \texttt{prefactor\_mode}
column: every row should show \texttt{frequency-resolved}. The
\texttt{run\_manifest.json} at the output root records
\texttt{"prefactor\_mode":\ "frequency-resolved"} in its
\texttt{parameters} block.
...
\tightlist
\item
  Peak molar absorptivity (\texttt{gaussian\_molar\_absorptivity\_max})
  will be slightly higher than the constant-prefactor run for the same
  \texttt{sigma\_ev} because the wavenumber factor shifts weight toward
  higher frequencies.
\item
