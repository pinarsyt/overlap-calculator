"""Frequency-resolved prefactor case study: drive both the CLI and the HTTP API.

Demonstrates switching the oscillator-strength → ε convention from the
default ``constant`` mode to ``frequency-resolved`` via
``--prefactor-mode frequency-resolved`` on the CLI and
``prefactor_mode=frequency-resolved`` on the API.
...
CLI_OPTIONS: list[str] = [
    ...
    "--prefactor-mode", "frequency-resolved",
    "--plot-dpi", "400",
]
