...
\title{Case Study 15 --- Vibronic Tabular Branch}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Pre-computed bandshapes through the experimental input branch}
\author{}
\date{}
...
\hypertarget{case-study-15--vibronic-tabular-branch}{
\section{Case Study 15 --- Vibronic Tabular
Branch}\label{case-study-15--vibronic-tabular-branch}}
...
The key insight: \texttt{overlap-calculator} treats every experimental
tabular input as absorbance \(A(\lambda)\) directly and does not broaden
it further. Any bandshape --- vibronic progression, empirical spectrum,
or computationally generated Franck--Condon envelope --- is handled
correctly, with no additional flags required.

The user-selectable knob is: use \texttt{input\_type = "experimental"}
in the manifest (or supply a CSV/XLSX to \texttt{generate-input}).
...
Studies 02, 03, and 11. The \texttt{input.json} selects 2 absorbance
series from the PhotochemCAD-derived spreadsheet. Each series is a real
UV/Vis absorption spectrum measured in solution --- it captures the true
bandshape including any vibronic structure already resolved by the
spectrometer.

\textbf{Citation.} The bundled workbook is derived from M. Taniguchi \&
J. S. Lindsey, \emph{Photochem. Photobiol.} \textbf{94} (2018)
290--327. See the project
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{overlap{-}calculator}\NormalTok{ analyze }\AttributeTok{{-}{-}input}\NormalTok{ case\_studies/15\_vibronic\_tabular\_branch/input.json }\AttributeTok{{-}{-}out}\NormalTok{ case\_studies/15\_vibronic\_tabular\_branch/output/cli }\AttributeTok{{-}{-}default{-}light{-}sources}\NormalTok{ AM15G,LEDB4 }\AttributeTok{{-}{-}plot{-}dpi}\NormalTok{ 400}
\end{Highlighting}
\end{Shaded}

After the run the \texttt{source\_type} column in \texttt{results.*}
reads \texttt{experimental} for every row, confirming that no broadening
was applied.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ case\_studies/15\_vibronic\_tabular\_branch/submit.py}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{7-what-this-demonstrates}{
\subsection{7. What this demonstrates}\label{7-what-this-demonstrates}}

\begin{itemize}
\tightlist
\item
  The absorbed-flux, absorbed-fraction, and shape-overlap descriptor
  pipeline is identical for theoretical and experimental inputs. The
  only difference is that experimental inputs skip the broadening step.
\item
  A vibronic spectrum stored in a CSV or XLSX file (wavelength column
  + absorbance column) is all that is needed. No extra flags are
  required.
\item
  The \texttt{shape\_overlap} metric in particular is
  broadening-agnostic: it compares the max-normalised shape of the
  supplied spectrum against the max-normalised light-source spectrum
  regardless of how the absorption bandshape was generated.
\end{itemize}
